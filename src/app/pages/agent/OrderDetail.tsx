...
export const OrderDetail = () => {
  ...
  const handleSendToWarehouse = () => {
    updateOrderStatus(order.id, 'tayyorlanmagan');
        ...
        <div className="p-4 space-y-4">
          ...
          {editMode ? (
            <div className="space-y-3">
              <h3 className="text-sm font-semibold text-gray-800">{t('orders.selectProducts')}</h3>
              {/* Search */}
              <div className="relative">
                <input
                  ...
                  className="w-full px-4 py-3 rounded-xl border border-gray-200 bg-white text-sm focus:outline-none focus:border-[#2563EB] focus:ring-2 focus:ring-blue-50"
                />
              </div>
              <div className="space-y-2">
                {filteredProducts.map(product => {
                  ...
                  return (
                    <div key={product.id} className={`bg-white rounded-xl p-3 border-2 transition-all ${qty > 0 ? 'border-[#2563EB]' : 'border-gray-200'}`}>
                      <div className="flex items-center gap-3">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <p className="text-xs text-[#2563EB] font-semibold mt-0.5">{formatCurrency(product.price)}</p>
                        </div>
                        <div className="flex items-center gap-1.5">
                          <button
                            onClick={() => setEditQty(product.id, product.name, product.price, -1)}
                            disabled={qty === 0}
                            className="w-7 h-7 rounded-lg border border-gray-200 flex items-center justify-center disabled:opacity-30 active:bg-gray-100"
                          >
                            <Minus size={12} />
                          </button>
                          <input
                            ...
                            placeholder="0"
                            className="w-10 h-7 text-center text-sm font-bold border border-gray-200 rounded-lg focus:outline-none focus:border-[#2563EB] [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none"
                          />
                          <button
                            onClick={() => setEditQty(product.id, product.name, product.price, 1)}
              ...
              <div className="flex gap-2 pt-1">
                <button
                  ...
                  className="flex-1 py-3 rounded-xl border border-gray-200 text-gray-600 font-medium text-sm flex items-center justify-center gap-1.5"
                >
                  <X size={15} />
                  {t('common.cancel')}
                </button>
