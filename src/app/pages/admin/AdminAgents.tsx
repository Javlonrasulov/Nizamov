...
import { Plus, Phone, TrendingUp, CalendarDays, Edit2, Truck, Trash2, Check, X } from 'lucide-react';
...
import { apiGetVehicles, apiCreateVehicle, apiDeleteVehicle, apiUpdateVehicle } from '../../api/vehicles';
...
export const AdminAgents = () => {
  ...
  const openEdit = (u: User) => {
    ...
    setForm({
      ...
      vehicleName: u.vehicleName && vehiclesList.some(v => v.name === u.vehicleName) ? u.vehicleName : (u.vehicleName ? t('admin.staff.form.vehicleOther') : ''),
      vehicleOther: u.vehicleName && !vehiclesList.some(v => v.name === u.vehicleName) ? u.vehicleName : '',
      comment: u.comment || '',
    });
    setShowForm(true);
  };
  ...
  const handleAddVehicle = async () => {
    const name = vehicleFormName.trim();
    if (!name) return;
    ...
    try {
      ...
      setVehicleActionError(t('admin.staff.vehicles.saveError'));
    }
  };
  ...
  const handleUpdateVehicle = async () => {
    ...
    const oldVehicle = vehiclesList.find(v => v.id === editingVehicleId);
    setVehicleActionError('');
    try {
      const updated = await apiUpdateVehicle(editingVehicleId, editingVehicleName);
      await loadVehicles();
      if (oldVehicle && form.vehicleName === oldVehicle.name) {
        setForm(prev => ({ ...prev, vehicleName: updated.name }));
      }
      cancelVehicleEdit();
    } catch {
      setVehicleActionError(t('admin.staff.vehicles.saveError'));
  ...
  const handleDeleteVehicle = async () => {
    ...
    setVehicleActionError('');
    try {
      await apiDeleteVehicle(vehicleDeleteTarget.id);
      if (form.vehicleName === vehicleDeleteTarget.name) {
        setForm(prev => ({ ...prev, vehicleName: '', vehicleOther: '' }));
      }
      if (editingVehicleId === vehicleDeleteTarget.id) {
        cancelVehicleEdit();
      }
      setVehicleDeleteTarget(null);
      await loadVehicles();
    } catch {
      setVehicleActionError(t('admin.staff.vehicles.deleteError'));
    }
  };
  ...
  const resolveVehicle = () => (form.vehicleName === t('admin.staff.form.vehicleOther') ? form.vehicleOther.trim() : form.vehicleName) || undefined;
      ...
      <div className="space-y-4 sm:space-y-5">
        <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
          <div className="min-w-0">
            <h1 className="text-lg sm:text-xl font-bold text-gray-900 dark:text-white break-words">{t('admin.agentsPage')}</h1>
            <p className="text-sm text-gray-500 dark:text-gray-400 mt-0.5">{staffList.length} {t('common.pcs')}</p>
          </div>
          <div className="flex w-full lg:w-auto flex-col sm:flex-row sm:flex-wrap gap-2">
            {hasFilter && (
            ...
            <button
              onClick={() => { setShowVehiclesModal(true); setVehicleActionError(''); void loadVehicles(); }}
              className="inline-flex w-full sm:w-auto items-center justify-center gap-2 px-4 py-2.5 rounded-xl border border-gray-200 dark:border-gray-600 text-gray-600 dark:text-gray-300 text-sm font-medium hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors"
            >
              <Truck size={15} />
              {t('admin.staff.manageVehicles')}
            </button>
            <button
              onClick={openAdd}
              className="inline-flex w-full sm:w-auto items-center justify-center gap-2 px-4 py-2.5 rounded-xl bg-[#2563EB] text-white text-sm font-medium hover:bg-blue-700 transition-colors shadow-sm shadow-blue-200"
            >
              <Plus size={15} />
              {t('admin.addAgent')}
            </button>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-4">
              ...
              <div>
                ...
                <input
                  ...
                  placeholder={t('admin.staff.form.phonePlaceholder')}
                  className="w-full px-3 py-2.5 rounded-xl border border-gray-200 dark:border-gray-600 bg-gray-50 dark:bg-gray-700 text-gray-900 dark:text-white text-sm focus:outline-none focus:border-[#2563EB]"
                />
              </div>
              <div>
              ...
              <div className="mt-4">
                ...
                <div className="flex flex-col sm:flex-row flex-wrap gap-2 items-stretch sm:items-center">
                  <select
                    ...
                    className="crm-select w-full sm:w-auto px-3 py-2.5 min-w-[180px]"
                  >
                    <option value="">{t('admin.staff.form.vehicleSelect')}</option>
                    {vehiclesList.map(v => (
                      <option key={v.id} value={v.name}>{v.name}</option>
                    ))}
                    <option value={t('admin.staff.form.vehicleOther')}>{t('admin.staff.form.vehicleOther')}</option>
                  </select>
                  <button
                    type="button"
                    onClick={() => { setShowVehiclesModal(true); setVehicleActionError(''); void loadVehicles(); }}
                    className="inline-flex w-full sm:w-auto items-center justify-center gap-1 px-3 py-2.5 rounded-xl border border-dashed border-gray-300 dark:border-gray-600 text-gray-600 dark:text-gray-400 text-sm hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors"
                  >
                    <Truck size={14} />
                    {t('admin.staff.manageVehicles')}
                  </button>
                  {form.vehicleName === t('admin.staff.form.vehicleOther') && (
                    <input
                      type="text"
                      value={form.vehicleOther}
                      onChange={e => setForm(p => ({ ...p, vehicleOther: e.target.value }))}
                      placeholder={t('admin.staff.form.vehicleCustomPlaceholder')}
                      className="w-full sm:w-auto px-3 py-2.5 rounded-xl border border-gray-200 dark:border-gray-600 bg-gray-50 dark:bg-gray-700 text-gray-900 dark:text-white text-sm focus:outline-none focus:border-[#2563EB] min-w-[160px]"
                    />
                  )}
                </div>
              </div>
        ...
        {showVehiclesModal && (
          <div className="fixed inset-0 z-50 p-3 sm:p-4 flex items-center justify-center">
            <div className="absolute inset-0 bg-black/50 dark:bg-black/70" onClick={() => { setShowVehiclesModal(false); setVehicleDeleteTarget(null); cancelVehicleEdit(); setVehicleFormName(''); }} />
            <div className="relative w-full max-w-2xl max-h-[90vh] bg-white dark:bg-gray-800 rounded-2xl border border-gray-100 dark:border-gray-700 shadow-xl overflow-hidden">
              <div className="flex items-start justify-between gap-3 p-4 sm:p-5 border-b border-gray-100 dark:border-gray-700">
                <div className="min-w-0">
                  <h3 className="text-lg font-bold text-gray-900 dark:text-white">{t('admin.staff.vehicles.title')}</h3>
                  <p className="text-sm text-gray-500 dark:text-gray-400 mt-1 break-words">
                    {t('admin.staff.vehicles.subtitle')}
                  </p>
                </div>
                <button
                  onClick={() => { setShowVehiclesModal(false); setVehicleDeleteTarget(null); cancelVehicleEdit(); setVehicleFormName(''); }}
                  className="shrink-0 p-2 rounded-xl text-gray-500 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-700"
                >
                  <X size={18} />
                </button>
              </div>

              <div className="p-4 sm:p-5 space-y-4 overflow-y-auto max-h-[calc(90vh-88px)]">
                {vehicleActionError && (
                  <div className="p-3 rounded-xl border border-red-200 dark:border-red-900/40 bg-red-50 dark:bg-red-900/20 text-sm text-red-700 dark:text-red-300">
                    {vehicleActionError}
                  </div>
                ...
                <div className="rounded-2xl border border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-900/40 p-3 sm:p-4">
                  <div className="flex flex-col sm:flex-row gap-2">
                    <input
                      type="text"
                      value={vehicleFormName}
                      onChange={e => setVehicleFormName(e.target.value)}
                      placeholder={t('admin.staff.vehicles.inputPlaceholder')}
                      className="flex-1 px-3 py-2.5 rounded-xl border border-gray-200 dark:border-gray-600 bg-white dark:bg-gray-800 text-gray-900 dark:text-white text-sm focus:outline-none focus:border-[#2563EB]"
                    />
                    <button
                      onClick={handleAddVehicle}
                      disabled={!vehicleFormName.trim()}
                      className="inline-flex w-full sm:w-auto items-center justify-center gap-2 px-4 py-2.5 rounded-xl bg-[#2563EB] text-white text-sm font-medium hover:bg-blue-700 disabled:opacity-50 disabled:cursor-not-allowed"
                    >
                      <Plus size={15} />
                ...
                {vehiclesList.length === 0 ? (
                  <div className="rounded-2xl border border-dashed border-gray-300 dark:border-gray-600 p-8 text-center">
                    <Truck size={22} className="mx-auto mb-2 text-gray-300 dark:text-gray-500" />
                    <p className="text-sm text-gray-500 dark:text-gray-400">{t('admin.staff.vehicles.empty')}</p>
                  </div>
                ) : (
                  <div className="space-y-3">
                    {vehiclesList.map(vehicle => (
                      <div
                        key={vehicle.id}
                        className="rounded-2xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900/40 p-3 sm:p-4"
                      >
                          ...
                          <div className="min-w-0 flex-1">
                            {editingVehicleId === vehicle.id ? (
                              <input
                                ...
                                placeholder={t('admin.staff.vehicles.inputPlaceholder')}
                                className="w-full px-3 py-2.5 rounded-xl border border-gray-200 dark:border-gray-600 bg-gray-50 dark:bg-gray-800 text-gray-900 dark:text-white text-sm focus:outline-none focus:border-[#2563EB]"
                              />
                            ) : (
                              <>
                                <div className="text-sm font-semibold text-gray-900 dark:text-white break-words">{vehicle.name}</div>
                              </>
                            )}
                          </div>
                            ...
                            {editingVehicleId === vehicle.id ? (
                              <>
                                <button
                                  ...
                                  disabled={!editingVehicleName.trim()}
                                  className="inline-flex w-full sm:w-auto items-center justify-center gap-1.5 px-3 py-2 rounded-xl bg-[#2563EB] text-white text-sm font-medium hover:bg-blue-700 disabled:opacity-50"
                                >
                                  <Check size={14} />
                                  {t('common.save')}
                                </button>
                                <button
                                  ...
                                  className="inline-flex w-full sm:w-auto items-center justify-center gap-1.5 px-3 py-2 rounded-xl border border-gray-200 dark:border-gray-600 text-gray-700 dark:text-gray-200 text-sm font-medium hover:bg-gray-50 dark:hover:bg-gray-700"
                                >
                                  <X size={14} />
                                  {t('common.cancel')}
                                </button>
                              ...
                              <>
                                ...
                                <button
                                  onClick={() => setVehicleDeleteTarget(vehicle)}
                                  className="inline-flex w-full sm:w-auto items-center justify-center gap-1.5 px-3 py-2 rounded-xl bg-red-600 text-white text-sm font-medium hover:bg-red-700"
                                >
                                  <Trash2 size={14} />
                                  {t('common.delete')}
                                </button>
        ...
        {vehicleDeleteTarget && (
          <div className="fixed inset-0 z-[60] flex items-center justify-center p-4">
            <div className="absolute inset-0 bg-black/40" onClick={() => setVehicleDeleteTarget(null)} />
            <div className="relative w-full max-w-md bg-white dark:bg-gray-800 rounded-2xl border border-gray-100 dark:border-gray-700 shadow-xl">
              <div className="p-4 border-b border-gray-100 dark:border-gray-700">
                <div className="font-semibold text-gray-900 dark:text-white">{t('admin.staff.vehicles.deleteTitle')}</div>
                <div className="text-xs text-gray-500 dark:text-gray-400 mt-1 break-words">{vehicleDeleteTarget.name}</div>
              </div>
              <div className="p-4">
                <p className="text-sm text-gray-700 dark:text-gray-200">{t('admin.staff.vehicles.deleteConfirm')}</p>
              </div>
              <div className="p-4 border-t border-gray-100 dark:border-gray-700 flex flex-col sm:flex-row items-stretch sm:items-center justify-end gap-2">
                <button
                  onClick={() => setVehicleDeleteTarget(null)}
            ...
            {staffList.map((user, idx) => {
              ...
              return (
                <div key={user.id} className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-100 dark:border-gray-700 shadow-sm overflow-hidden hover:shadow-md transition-shadow">
                  <div className={`bg-gradient-to-r ${colors[idx % colors.length]} p-4 sm:p-5 text-white flex items-start justify-between gap-3`}>
                    <div className="flex items-start gap-3 min-w-0">
                      <div className="w-11 h-11 sm:w-12 sm:h-12 rounded-full bg-white/20 flex items-center justify-center shrink-0">
                      ...
                      <div className="min-w-0">
                        ...
                        <div className="flex items-center gap-1.5 mt-0.5">
                          ...
                          <span className="text-white/80 text-xs break-all">{user.phone}</span>
                        </div>
                        <span className="inline-block mt-1 px-2 py-0.5 rounded-md bg-white/20 text-xs break-words">
                          {t(`login.role.${user.role}` as any)}
                        </span>
                        {user.vehicleName && (
                          <div className="flex items-center gap-1 mt-1 text-white/90 text-xs break-words">
                            <Truck size={10} className="shrink-0" />
                            <span className="break-words">{user.vehicleName}</span>
                          </div>
                        )}
                        {user.comment?.trim() && (
                          <div className="mt-2 max-w-[220px] sm:max-w-[240px] rounded-lg bg-white/15 px-2.5 py-1.5 text-xs text-white/95 break-words">
