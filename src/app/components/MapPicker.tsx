...
export const MapPicker = ({ initialLat, initialLng, onConfirm, onClose }: MapPickerProps) => {
  ...
  const handleSearch = async () => {
    ...
    try {
      const res = await fetch(
        `https://nominatim.openstreetmap.org/search?q=${encodeURIComponent(searchQuery + ' Navoiy')}&format=json&limit=1&countrycodes=uz`
      );
  ...
  const handleMyLocation = () => {
    ...
    navigator.geolocation.getCurrentPosition(
      ...
      () => {
        setLocating(false);
        if (leafletMapRef.current) {
          leafletMapRef.current.flyTo([DEFAULT_CENTER.lat, DEFAULT_CENTER.lng], 14, { duration: 1 });
        }
      },
  ...
  return (
    ...
    <div
      ...
      onClick={onClose}
    >
      {/* Modal sheet - fixed 88vh, flex column, no overflow on outer */}
      <div
      ...
      >
        ...
        <div className="relative flex-1 min-h-0">
          ...
          {/* Coordinates badge */}
          {position && (
            <div className="absolute bottom-3 left-3 z-[1000] bg-white/95 rounded-xl px-3 py-1.5 text-xs font-mono text-gray-700 shadow border border-gray-200 pointer-events-none">
