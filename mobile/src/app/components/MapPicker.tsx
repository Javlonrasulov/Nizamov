...
export const MapPicker = ({ initialLat, initialLng, onConfirm, onClose }: MapPickerProps) => {
  ...
  const initMap = useCallback(() => {
    ...
    const center = (initialLat && initialLng) ? [initialLat, initialLng] : [DEFAULT_CENTER.lat, DEFAULT_CENTER.lng];
    const map = L.map(mapContainerRef.current, { center, zoom: 14, zoomControl: false });
    L.control.zoom({ position: 'bottomright' }).addTo(map);
    L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png', { attribution: '© OpenStreetMap', maxZoom: 19 }).addTo(map);
    if (initialLat && initialLng) placeMarker(L, map, initialLat, initialLng);
    map.on('click', (e: any) => {
  ...
  const handleMyLocation = () => {
    ...
    navigator.geolocation.getCurrentPosition(
      ...
      () => { setLocating(false); if (leafletMapRef.current) leafletMapRef.current.flyTo([DEFAULT_CENTER.lat, DEFAULT_CENTER.lng], 14, { duration: 1 }); },
      ...
      <div className="w-full max-w-[430px] bg-white dark:bg-gray-800 rounded-t-3xl flex flex-col" style={{ height: '88vh', maxHeight: '88vh' }} onClick={e => e.stopPropagation()}>
        <div className="flex items-center justify-between px-4 py-3 border-b border-gray-100 dark:border-gray-700 shrink-0">
          <div className="flex items-center gap-2">
            ...
            <span className="font-semibold text-gray-900 dark:text-white">Xaritadan joy tanlash</span>
          </div>
        ...
        <div className="px-3 py-2.5 bg-gray-50 dark:bg-gray-700/50 border-b border-gray-100 dark:border-gray-700 shrink-0">
          <div className="flex gap-2 items-center">
            <div className="flex-1 relative">
              ...
              <input type="text" value={searchQuery} onChange={e => setSearchQuery(e.target.value)} onKeyDown={e => e.key === 'Enter' && handleSearch()} placeholder="Qidiruv: ko'cha, mahalla..." className="w-full pl-9 pr-3 py-2.5 rounded-xl border border-gray-200 dark:border-gray-600 bg-white dark:bg-gray-800 text-sm text-gray-900 dark:text-white focus:outline-none focus:border-[#2563EB] placeholder:text-gray-400" />
            </div>
            <button onClick={handleSearch} disabled={searching} className="h-10 w-10 rounded-xl bg-[#2563EB] text-white flex items-center justify-center hover:bg-blue-700 disabled:opacity-50 transition-colors shrink-0">
              {searching ? <div className="w-4 h-4 border-2 border-white border-t-transparent rounded-full animate-spin" /> : <Search size={15} />}
            </button>
            <button onClick={handleMyLocation} disabled={locating} title="Mening joylashuvim" className="h-10 w-10 rounded-xl border border-gray-200 dark:border-gray-600 bg-white dark:bg-gray-800 flex items-center justify-center hover:bg-blue-50 dark:hover:bg-gray-700 disabled:opacity-50 transition-colors shrink-0">
        ...
        <div className="px-4 py-1.5 bg-blue-50 dark:bg-blue-900/20 border-b border-blue-100 dark:border-blue-800 shrink-0">
          <p className="text-xs text-blue-600 dark:text-blue-400 text-center">📍 Xaritaga bosib aniq joyni belgilang</p>
        </div>
        ...
        <div className="px-4 py-3 bg-white dark:bg-gray-800 border-t border-gray-100 dark:border-gray-700 shrink-0">
          ...
          <button onClick={onClose} className="w-full py-2.5 rounded-xl border border-gray-200 dark:border-gray-600 text-gray-600 dark:text-gray-300 font-medium text-sm hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors">Bekor qilish</button>
        </div>
      </div>
    </div>
  );
