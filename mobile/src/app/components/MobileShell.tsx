...
export const MobileShell = ({ children }: MobileShellProps) => {
  useEffect(() => {
    const setViewportVars = () => {
      ...
      document.documentElement.style.setProperty('--vv-bottom', `${Math.round(bottomInset)}px`);
    };
  ...
  return (
    <div
      className="min-h-0 md:min-h-screen md:max-h-none bg-gray-200 dark:bg-gray-950 flex items-start justify-center py-0 md:py-8 transition-colors duration-300"
      style={{ height: 'var(--app-height, 100dvh)', maxHeight: 'var(--app-height, 100dvh)' }}
    >
      <div
        className="w-full max-w-[390px] min-h-0 md:min-h-0 md:h-[844px] md:max-h-[844px] bg-[#F8FAFC] dark:bg-gray-900 md:rounded-[40px] md:shadow-2xl overflow-hidden flex flex-col md:border-4 md:border-gray-800 dark:md:border-gray-700 transition-colors duration-300"
        style={{ height: '100%' }}
      >
        {children}
      </div>
...
export const MobileHeader = ({ title, showBack = false, showLang = true, showLogout = false, rightElement }: MobileHeaderProps) => {
  ...
  return (
    <div className="fixed top-0 left-0 right-0 z-50 flex justify-center px-0 pointer-events-none">
      <div
        className="w-full max-w-[390px] bg-white dark:bg-gray-800 border-b border-gray-100 dark:border-gray-700 px-4 py-3 flex items-center justify-between transition-colors duration-300 pointer-events-auto"
...
export const MobileContent = ({ children, className = '' }: MobileContentProps) => {
  return (
    <div
      className={`flex-1 min-h-0 overflow-y-auto [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none] ${className}`}
