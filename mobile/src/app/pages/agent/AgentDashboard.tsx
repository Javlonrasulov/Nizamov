...
import { ShoppingBag, TrendingUp, Plus, UserPlus, ArrowRight } from 'lucide-react';
...
export const AgentDashboard = () => {
  const { t, currentUser, orders, refetchData, apiConnected } = useApp();
  ...
  return (
    <MobileShell>
      <MobileHeader title={t('agent.dashboard.title')} showLogout showLang />
      <MobileContent className="pb-20">
        <div className="p-4 space-y-4">
          ...
          <div className="bg-gradient-to-br from-[#2563EB] to-blue-700 rounded-2xl p-4 text-white">
            ...
            <p className="text-blue-200 text-xs mt-1">{new Date().toLocaleDateString('uz-UZ', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' })}</p>
            <p className="text-sm mt-2 font-medium text-white/90">
              {apiConnected ? '✓ Backend ulandi' : '✗ Ulanmagan'}
            </p>
          </div>
