...
export const CreateOrder = () => {
  ...
  const setQty = (product: Product, delta: number) => {
    setSelectedItems(prev => {
      ...
      if (existing) {
        const max = Math.max(0, product.stock ?? 0);
  ...
  const setQtyManual = (product: Product, val: string) => {
    ...
    const max = Math.max(0, product.stock ?? 0);
    if (qty > max) {
      setStockWarn(prev => ({ ...prev, [product.id]: true }));
      setTimeout(() => setStockWarn(prev => ({ ...prev, [product.id]: false })), 5000);
    }
    const clamped = Math.min(max, qty);
    setSelectedItems(prev => {
      if (clamped === 0) return prev.filter(i => i.product.id !== product.id);
      const existing = prev.find(i => i.product.id === product.id);
      if (existing) return prev.map(i => i.product.id === product.id ? { ...i, quantity: clamped } : i);
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <div className="flex items-center gap-1.5">
                          ...
                          <input
                            ...
                            onBlur={() => setQtyManual(product, String(qty))}
                            placeholder="0"
                          ...
                          <button
                            onClick={() => setQty(product, 1)}
                            disabled={!canPlus}
                            className="w-7 h-7 rounded-lg bg-[#2563EB] text-white flex items-center justify-center active:bg-blue-700 disabled:opacity-30 disabled:cursor-not-allowed"
                          >
                            <Plus size={12} />
                          </button>
                        </div>
